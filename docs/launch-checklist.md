...
- [x] Vercel Production 배포 확인: `https://gwanje.vercel.app`
...
알림톡(카카오톡) 실발송은 Solapi 연동으로 구현됐다. 요금은 워크스페이스별로
각자 연결한 Solapi 계정에서 차감된다(`alimtalk_settings`에 암호화 저장).
이메일 채널은 아직 미구현이다.

- [x] 발송 채널 결정: 알림톡 우선(이메일은 후순위)
- [x] 알림톡 제공사/API 결정 — Solapi(카카오 공식 딜러사)
- [ ] 이메일 제공사/API 결정
- [x] 발신 프로필 검증 정책 결정 — 워크스페이스별 BYO 계정, 저장 시 잔액 조회로 키 검증
- [x] 즉시 발송 작업 구현 — `after()` 백그라운드 트리거 + cron 안전망
- [x] 예약 발송 작업 구현 — `/api/campaigns/cron` 5분 주기
- [x] 실패/재시도/부분 성공 상태 설계 — `recipient_status` enum, 최대 3회 시도, 부분 실패도 캠페인은 종료
- [x] `campaign_recipient`에 실제 도달/실패 사유 저장 — 상태·사유·provider id·시도 횟수
- [ ] 테스트 수신처로 E2E 발송 확인 ← **운영 전 필수**

운영 전 준비(각 컨설팅사 1회): 카카오톡 채널 개설·비즈니스 인증 → Solapi 채널
연동(pfId) → 발신번호 등록 → 템플릿 검수(영업일 1~3일) → 잔액 충전 →
설정 → 알림톡 발송에서 연동. 서버는 `ALIMTALK_ENABLED`·`ALIMTALK_CRED_KEY`·
`ALIMTALK_CRON_SECRET` 필요(`.env.example` 참조).
