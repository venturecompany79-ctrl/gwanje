...
// 키: GOOGLE_TOKEN_ENCRYPTION_KEY (base64 인코딩된 32바이트). 서버 전용.

const IV_LENGTH = 12; // GCM 권장 nonce 길이

function getKey(): Buffer {
  const { encryptionKey } = getGoogleDriveConfig();
  const key = Buffer.from(encryptionKey, "base64");
  if (key.length !== 32) {
    throw new Error(
      "GOOGLE_TOKEN_ENCRYPTION_KEY는 base64로 인코딩한 32바이트여야 합니다. 예: openssl rand -base64 32",
    );
  }
  return key;
}
