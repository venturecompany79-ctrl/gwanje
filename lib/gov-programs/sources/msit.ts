import { getDataGoKrApiKey } from "@/lib/gov-programs/config";
import {
  buildUrl,
  combineText,
  fetchPagedItems,
  firstString,
  inferSupportField,
  parseDate,
  parseDateRange,
  splitTags,
  type JsonRecord,
} from "@/lib/gov-programs/http";
import {
  makeContentKey,
  type NormalizedProgram,
  type SourceAdapter,
} from "@/lib/gov-programs/types";

function normalize(row: JsonRecord, index: number): NormalizedProgram | null {
  // TODO: verify field names after data.go.kr 15074634 utilization approval.
  const title = firstString(row, [
    "title",
    "pblancNm",
    "ancmNm",
    "nttSj",
    "사업명",
    "제목",
  ]);
  if (!title) return null;

  const externalId =
    firstString(row, ["id", "pblancId", "ancmId", "nttId", "공고번호"]) ??
    `msit-${index}`;
  const period = firstString(row, ["reqstBeginEndDe", "aplyPd", "신청기간"]);
  const range = parseDateRange(period);
  const targetText = combineText(
    firstString(row, ["trgetNm", "지원대상"]),
    firstString(row, ["bsnsSumryCn", "summary", "사업개요"]),
    firstString(row, ["cn", "content", "내용"]),
  );
  const supportFieldText = firstString(row, ["supportField", "지원분야", "bizType"]);
  const postedDate = parseDate(firstString(row, ["regDt", "creatPnttm", "등록일"]));
  const program: NormalizedProgram = {
    source: "msit",
    externalId,
    contentKey: "",
    title,
    supportField: inferSupportField(supportFieldText, title, targetText) ?? "기술",
    orgName: firstString(row, ["orgName", "jrsdInsttNm", "담당부서", "작성자"]),
    targetText,
    hashtags: splitTags(combineText(supportFieldText, firstString(row, ["hashtags", "kwrd"]))),
    region: firstString(row, ["areaNm", "지역"]),
    applyStart: range.start ?? postedDate,
    applyEnd: range.end,
    detailUrl: firstString(row, ["dtlUrl", "pblancUrl", "fileUrl", "url"]),
    raw: row,
  };
  return { ...program, contentKey: makeContentKey(program) };
}

export const msitAdapter: SourceAdapter = {
  code: "msit",
  async fetch() {
    const key = getDataGoKrApiKey();
    if (!key) return [];

    const rows = await fetchPagedItems((pageNo) =>
      buildUrl(
        "https://apis.data.go.kr/1721000/MSITPublicNoticeService/getPublicNoticeList",
        {
          pageNo,
          numOfRows: 100,
          type: "json",
        },
        { serviceKey: key },
      ),
    );
    return rows
      .map((row, index) => normalize(row, index))
      .filter((program): program is NormalizedProgram => program !== null);
  },
};
